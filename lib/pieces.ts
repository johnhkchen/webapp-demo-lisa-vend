/**
 * Shape data for the seven standard pieces (I, O, T, S, Z, J, L) and their four SRS
 * rotation states.
 *
 ...
 * sequencing is the sibling ticket T-002-01-03 — it consumes `PIECE_TYPES` below but owns
 * the bag logic; `PIECE_TYPES` here is just the type alphabet expressed as an ordered list.
 */

import type { Point, PieceType, RotationState } from "./types";
 ...
 */
export const BOUNDING_BOX: Readonly<Record<PieceType, number>> = {
  I: 4,
  O: 2,
  T: 3,
  S: 3,
...
/**
 * The core table: `PIECE_CELLS[type][rotation]` is the four occupied cell offsets for that
 ...
 */
export const PIECE_CELLS: Readonly<
  Record<PieceType, readonly (readonly Point[])[]>
> = {
 ...
 * "where is this piece right now". Equivalent to `PIECE_CELLS[type][rotation]`.
 */
